...
사용자는 팀 기획서를 작성하고, AI가 DMAP 표준에 맞춰 플러그인을 자동 생성함.

**가이드 구성:**

| 구분 | 내용 | 대상 |
|------|------|------|
| 팀 기획서 | 사용자가 작성하는 플러그인 기획 양식 | 사용자 |
...
작성한 팀 기획서를 DMAP 빌더에 전달하는 방법.

**방법 A: AI 도움으로 작성 (추천)**

기본정보, 핵심기능, 사용자 플로우만 알면 AI가 에이전트 구성 힌트와 참고 공유 자원을 자동 완성함:

```
/dmap:team-planner
```

완성된 기획서는 `{dmap 프로젝트}/output/team-plan-{플러그인명}.md`에 저장되며,
`/dmap:develop-plugin` 실행 시 자동으로 활용됨.

**방법 B: 직접 작성**

**Step 1.** 팀 기획서를 마크다운 파일(`.md`)로 작성
...
Claude Code 프롬프트 입력창에 요구사항 파일을 **드래그 앤 드롭**하여 업로드.

**Step 4.** 플러그인 개발 명령 입력
...
팀 기획서를 분석하고, 부족한 정보를 사용자에게 문의하여 수집.

**Step 1. 요구사항 파악**

팀 기획서의 각 항목을 분석하고, 핵심기능과 사용자 플로우의 일관성 확인.
...
사용자 문의를 통해 수집된 정보를 팀 기획서에 반영.
원본 기획서의 누락/모호 항목을 보완하고, 새로 확인된 사항을 추가하여 완성된 기획서로 갱신.

결과 파일: `{플러그인 프로젝트}/output/team-plan-{플러그인명}.md`
...
결과 파일: `{플러그인 프로젝트}/output/develop-plan.md`
