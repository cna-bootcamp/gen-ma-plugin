...
| 플러그인 | 외부호출스킬(External)에서 위임할 대상 플러그인의 명세서 | `resources/plugins/{2차분류}/` |
...
[부록 B](#부록-b-ai-활용-프롬프트)의 프롬프트를 AI에게 전달하여 자동 등록.
...
플러그인 명세서는 외부호출스킬(External)이 대상 플러그인을 참조하기 위한 인터페이스 문서임.
...
>
> ```markdown
> | ai-agent | my-plugin | AI 에이전트 개발 자동화 플러그인 | [상세](plugins/ai-agent/my-plugin.md) |
> ```
...
[부록 B](#부록-b-ai-활용-프롬프트)의 툴 가이드 작성 프롬프트를 AI에게 전달하면
소스 코드를 읽고 아래 템플릿에 맞춰 자동 생성함.

> **참고 — 출력 경로:** `resources/tools/{tool-name}.md` (파일명은 kebab-case)
...
MCP 서버는 외부 npm 패키지 또는 커스텀 구현으로 제공됨.
소스 배치, 툴 가이드 작성, 카탈로그 등록 3단계로 진행.
...
[부록 B](#부록-b-ai-활용-프롬프트)의 툴 가이드 작성 프롬프트를 AI에게 전달하면
소스 코드 또는 패키지 정보를 읽고 아래 템플릿에 맞춰 자동 생성함.
