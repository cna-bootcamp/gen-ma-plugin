...
`{PLUGIN_DIR}/AGENTS.md`의 `AI_RUNTIME` 값을 확인하여 해당 런타임의 호출 방식을 사용할 것.
...
스킬이 호출될 때마다 아래 순서로 현재 런타임을 감지하고 `{PLUGIN_DIR}/AGENTS.md`의 `AI_RUNTIME` 값을 업데이트.
