...
사용자가 `/dmap:add-ext-skill`로 호출하여
...
- `.dmap/dmap/requirements.md` (요구사항 정의서)
...
| 워크플로우 | Phase 0: 선행 확인, Phase 1: 컨텍스트 수집, Phase 2: 경로 분기, Phase 3: 외부 스킬 위임, Phase 4: 결과 검증 |
| 완료 조건 | 체크리스트 |
| 검증 프로토콜 | 산출물 검증 방법 |
| 상태 정리 | 임시 파일 처리 |
| 취소/재개 | 중단·재시작 방법 |
| 스킬 부스팅 | OMC 스킬 활용 테이블 |
| MUST 규칙 | 필수 규칙 테이블 |
